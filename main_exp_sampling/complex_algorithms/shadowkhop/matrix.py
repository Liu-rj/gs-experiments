...
def sample_w_o_relabel(P: gs.Matrix, fanouts, seeds, seeds_ptr):
    ...
    output_node = seeds
    blocks = []
    torch.cuda.nvtx.range_push('neighbor sample')
    ...
    subg = graph._CAPI_slicing(seeds, 0, gs._CSC, gs._CSC + gs._COO, False)
    indptr, indices, eid = subg._CAPI_get_csc()
    coo_ptr = indptr[seeds_ptr]
    coo_row, coo_col, eid = subg._CAPI_get_coo()
    sub_coo_row, sub_coo_col, sub_coo_ptr = torch.ops.gs_ops.BatchCOOSlicing(
        1, coo_row, coo_col, coo_ptr, seeds, seeds_ptr)
    unique_tensor, unique_tensor_ptr, sub_coo_row, sub_coo_col, sub_coo_ptr = torch.ops.gs_ops.BatchCOORelabel(
        seeds, seeds_ptr, seeds[sub_coo_col], sub_coo_row, sub_coo_ptr)
