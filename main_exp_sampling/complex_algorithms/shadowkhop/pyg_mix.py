...
def train(args, dataset):
    device = args.device
    if device == 'cpu':
        kwargs = {'batch_size': args.batchsize, 'num_workers': 6, 'persistent_workers': True}
    else:
        kwargs = {'batch_size': args.batchsize, 'num_workers': 0}

    graph, train_idx = dataset
    graph = graph.to(device)
    
    train_loader = ShaDowKHopSampler(graph, depth=2, num_neighbors=10,
                                 node_idx=train_idx, replace=False, **kwargs)
    print('####################################################START')
    epoch_time = []
    epoch_transfer_time = []
    mem_list = []
    ...
    for epoch in range(args.epoch):
        ...
        print("Epoch {:05d} | Epoch Sample Time {:.4f} s | Epoch Transfer Time {:.4f} s | GPU Mem Peak {:.4f} GB"
              .format(epoch, epoch_time[-1], epoch_transfer_time[-1], mem_list[-1]))
