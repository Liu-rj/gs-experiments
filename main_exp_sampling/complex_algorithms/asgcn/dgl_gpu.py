...
def benchmark_w_o_relabel(args, graph, nid, fanouts, n_epoch, adj_weight, features, W):
    print('####################################################DGL w/o relabel')
    sampler = ASGCNSampler(fanouts, replace=False, use_uva=args.use_uva,
                           eweight=adj_weight, node_feats=features, W=W)
...
def benchmark_w_relabel(args, graph, nid, fanouts, n_epoch, adj_weight, features, W):
    print('####################################################DGL w relabel')
    sampler = ASGCNSamplerRelabel(fanouts, replace=False, use_uva=args.use_uva,
                                  eweight=adj_weight, node_feats=features, W=W)
    seedloader = SeedGenerator(
        nid, batch_size=args.batchsize, shuffle=True, drop_last=False)
...
def train(dataset, args):
    ...
    if args.use_uva and device == 'cpu':
        ...
        g = g.to(device)
        weight, features = weight.to(device), features.to(device)

    n_epoch = 6
    benchmark_w_o_relabel(args, g, train_nid, fanouts, n_epoch, weight, features, W)
    benchmark_w_relabel(args, g, train_nid, fanouts, n_epoch, weight, features, W)
