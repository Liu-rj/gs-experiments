...
def sample_w_o_relabel(A: gs.Matrix, seeds, fanouts, features, W_1, W_2, sample_a, use_uva):
    ...
    for fanout in fanouts:
        ...
        att3 = subg._CAPI_normalize(
            0, gs._CSC)._CAPI_get_data("default").unsqueeze(1)
        ...
        subg = subA._graph._CAPI_sampling_with_probs(
            0, att, fanout, True, gs._CSC, gs._CSC)
...
def benchmark_w_o_relabel(args, graph, nid, fanouts, n_epoch, features, W1, W2, Wa):
    print('####################################################w/o relabel')
...
def benchmark_w_relabel(args, graph, nid, fanouts, n_epoch, features, W1, W2, Wa):
    print('####################################################w relabel')
    seedloader = SeedGenerator(
        nid, batch_size=args.batchsize, shuffle=True, drop_last=False)
...
def train(dataset, args):
    ...
    W1 = torch.nn.init.xavier_normal_(
        torch.Tensor(features.shape[1], 64)).to('cuda')
    W2 = torch.nn.init.xavier_normal_(
        torch.Tensor(features.shape[1], 64)).to('cuda')
    ...
    n_epoch = 6
    benchmark_w_o_relabel(args, m, train_nid, fanouts,
                          n_epoch, features, W1, W2, Wa)
    benchmark_w_relabel(args, m, train_nid, fanouts,
                        n_epoch, features, W1, W2, Wa)
