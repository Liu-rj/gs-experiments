...
class PASSSamplerRelabel(dgl.dataloading.BlockSampler):
    ...
    def sample_blocks(self, g, seed_nodes, exclude_eids=None):
        ...
        for fanout in reversed(self.fanouts):
            ...
            frontier = dgl.sampling.sample_neighbors(
                subg, local_seeds_nid, fanout, prob=att, replace=True)
...
def benchmark_w_o_relabel(args, graph, nid, fanouts, n_epoch, features, W1, W2, Wa):
    ...
    sampler = PASSSampler(fanouts, W_1=W1, W_2=W2, sample_a=Wa,
                          use_uva=args.use_uva, features=features)
...
def benchmark_w_relabel(args, graph, nid, fanouts, n_epoch, features, W1, W2, Wa):
    print('####################################################DGL w relabel')
    sampler = PASSSamplerRelabel(
        fanouts, W_1=W1, W_2=W2, sample_a=Wa, use_uva=args.use_uva, features=features)
    seedloader = SeedGenerator(
        nid, batch_size=args.batchsize, shuffle=True, drop_last=False)
