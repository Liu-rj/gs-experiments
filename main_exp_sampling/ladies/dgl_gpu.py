...
def benchmark_w_o_relabel(args, graph, nid, fanouts, n_epoch, W):
    print('####################################################DGL w/o relabel')
    sampler = LADIESSampler(fanouts, weight='weight',
                            out_weight='w', replace=False, W=W, use_uva=args.use_uva)
...
def benchmark_w_relabel(args, graph, nid, fanouts, n_epoch, W):
    print('####################################################DGL w relabel')
    sampler = LADIESSamplerRelabel(
        fanouts, weight='weight', out_weight='w', replace=False, W=W, use_uva=args.use_uva)
    seedloader = SeedGenerator(
        nid, batch_size=args.batchsize, shuffle=True, drop_last=False)
...
def train(dataset, args):
    ...
    n_epoch = 6
    benchmark_w_o_relabel(args, g, train_nid, fanouts, n_epoch, weight)
    benchmark_w_relabel(args, g, train_nid, fanouts, n_epoch, weight)
