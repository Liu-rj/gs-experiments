...
__host__ __device__
    EdgePos_t
    newNeighborsSize(int hop, EdgePos_t num_edges)
{
                               ...
                               EdgePos_t *collectiveNeighborhoodCSRCols, curandState *randStates)
{
  ...
  if (transit != invalidVertex)
  {
    ...
    else if (tpMode == CollectiveNeighborhoodSize)
    {
      ...
      ::atomicAdd((unsigned long long *)&sampleNeighborhoodSizes[(sampleIdx - deviceFirstSample)], (unsigned long long)numTransitEdges);
    }
  }
                              ...
                              const EdgePos_t subWarpKernelTBPositionsNum)
{
  ...
  for (int transitI = 0; transitI < TRANSITS_PER_THREAD; transitI++)
  {
    ...
    if (false)
    {
      // shMem.edgeAndWeightCache[threadIdx.x%32] = numTransitEdges + (int64_t)maxWeight + (int64_t)glTransitEdges + (int64_t)glTransitEdgeWeights;
      continue;
    }
...
#define NUM_THREAD_GROUPS (THREADS / LoadBalancing::LoadBalancingThreshold::BlockLevel)
  ...
  for (int fullBlockIdx = blockIdx.x; fullBlockIdx < totalThreadBlocks; fullBlockIdx += gridDim.x)
  {
    ...
    for (int transitI = 0; transitI < TRANSITS_PER_THREAD; transitI++)
    {
      ...
      if (true)
      { // || transit == shMem.transitForSubWarp[threadBlockWarpIdx]) {
                                       ...
                                       EdgePos_t *sumNeighborhoodSizes)
{
  ...
  for (int transitIdx = threadIdx.x; transitIdx < numTransits; transitIdx += blockDim.x)
  {
    ...
    {
      //!!! modified
      ::atomicAdd((unsigned long long *)&neighborhoodSize, (unsigned long long)csr->get_n_edges_for_vertex(transit));
    }
  }
  ...
  {
    //!!! modified
    sampleNeighborhoodPos[sampleIdx] = ::atomicAdd((unsigned long long *)sumNeighborhoodSizes, (unsigned long long)neighborhoodSize);
  }
}
                                           ...
                                           int *kernelTypeForTransit, VertexID_t *transitToSamplesKeys)
{
  ...
  const int64_t SHMEM_SIZE = 7 * TB_THREADS;
  ...
  // for (int kTy = 1; kTy < TransitKernelTypes::SubWarpKernel; kTy++)
  {
    ...
    if (threadIdx.x == blockDim.x - 1)
    {
      ...
      threadGroupsInsertionPos = ::atomicAdd((unsigned long long *)glKernelTransitsNum, (unsigned long long)totalThreadGroups);
    }
    __syncthreads();
...
bool allocNextDoorDataOnGPU(CSR *csr, NextDoorData<SampleType, App> &data)
{
  ...
  for (int64_t sampleIdx = 0; sampleIdx < App().numSamples(csr); sampleIdx++)
  {
    ...
    if ((EdgePos_t)initialVertices.size() != App().initialSampleSize(csr))
    {
      ...
      printf("initialSampleSize '%ld' != initialSample(%ld).size() '%ld'\n",
  ...
  for (auto deviceIdx = 0; deviceIdx < data.devices.size(); deviceIdx++)
  {
    ...
    gpu_memset(data.dFinalSamples[deviceIdx], (int64_t)data.INVALID_VERTEX, finalSampleSize * perDeviceNumSamples);
    ...
    if (curandDataSize < curandSizeLimit)
    {
      ...
      {
        maxSubWarpSize = max((int64_t)maxSubWarpSize, subWarpSizeAtStep<App>(s));
      }
...
bool doTransitParallelSampling(CSR *csr, NextDoorData<SampleType, App> &nextDoorData, bool enableLoadBalancing)
{
    ...
    else
    {
      ...
      else
      {
        ...
        for (auto deviceIdx = 0; deviceIdx < nextDoorData.devices.size(); deviceIdx++)
        {
          ...
          partitionTransitsInKernels<App, 1024, TransitKernelTypes::GridKernel, true><<<thread_block_size((*uniqueTransitNumRuns[deviceIdx]), (int64_t)1024), 1024>>>(step, dUniqueTransits[deviceIdx], dUniqueTransitsCounts[deviceIdx],
        ...
        if (useThreadBlockKernel and subWarpSize > 1)
        {
          for (auto deviceIdx = 0; deviceIdx < nextDoorData.devices.size(); deviceIdx++)
          {
            ...
            partitionTransitsInKernels<App, 1024, TransitKernelTypes::ThreadBlockKernel, false><<<thread_block_size((*uniqueTransitNumRuns[deviceIdx]), (int64_t)1024), 1024>>>(step, dUniqueTransits[deviceIdx], dUniqueTransitsCounts[deviceIdx],
                                                                                                                                                                                dTransitPositions[deviceIdx], *uniqueTransitNumRuns[deviceIdx], nextDoorData.INVALID_VERTEX, dGridKernelTransits[deviceIdx], dGridKernelTransitsNum[deviceIdx],
                                                                                                                                                                                dThreadBlockKernelTransits[deviceIdx], dThreadBlockKernelTransitsNum[deviceIdx], dSubWarpKernelTransits[deviceIdx], dSubWarpKernelTransitsNum[deviceIdx], nullptr,
                                                                                                                                                                                dIdentityKernelTransitsNum[deviceIdx], dKernelTypeForTransit[deviceIdx], nextDoorData.dTransitToSampleMapKeys[deviceIdx]);
  ...
  std::cout << "InversionTime: " << inversionTime << ", "
            << "LoadBalancingTime: " << loadBalancingTime << ", "
            << "GridKernelTime: " << gridKernelTime << ", ThreadBlockKernelTime: " << threadBlockKernelTime << ", SubWarpKernelTime: " << subWarpKernelTime << ", IdentityKernelTime: " << identityKernelTime << std::endl;
  for (auto deviceIdx = 0; deviceIdx < nextDoorData.devices.size(); deviceIdx++)
  {
    auto device = nextDoorData.devices[deviceIdx];
...
template <class SampleType, typename App>
torch::Tensor getFinalSamples(NextDoorData<SampleType, App> &nextDoorData)
{
  const size_t numSamples = nextDoorData.samples.size();
  const size_t finalSampleSize = getFinalSampleSize<App>();
  const size_t perDeviceNumSamples = numSamples / nextDoorData.devices.size();
  auto options = torch::TensorOptions().dtype(torch::kInt64).device(torch::kCUDA);
  torch::Tensor finalSamplesTensor;
  for (auto deviceIdx = 0; deviceIdx < nextDoorData.devices.size(); deviceIdx++)
  {
    auto device = nextDoorData.devices[deviceIdx];
    // Per Device Allocation
    CHK_CU(cudaSetDevice(device));
    auto samplesTensor = torch::from_blob(nextDoorData.dFinalSamples[deviceIdx], finalSampleSize * perDeviceNumSamples, options);
    if (deviceIdx == 0)
    {
      finalSamplesTensor = samplesTensor;
    }
    else
    {
      finalSamplesTensor = torch::cat({finalSamplesTensor, samplesTensor});
    }
  }

  return finalSamplesTensor.to(torch::kInt64);
}
              ...
              bool (*checkResultsFunc)(NextDoorData<SampleType, App> &))
{
  ...
  std::cout << "Graph has " << graph.get_n_edges() << " edges and " << graph.get_vertices().size() << " vertices " << std::endl;
