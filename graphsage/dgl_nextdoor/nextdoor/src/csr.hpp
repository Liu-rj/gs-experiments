...
public:
  ...
  __host__ __device__
      EdgePos_t
      get_start_edge_idx(VertexID vertex_id) const
  {
    if (!(vertex_id < n_vertices && 0 <= vertex_id))
    {
      printf("vertex_id %ld, n_vertices %d\n", vertex_id, n_vertices);
      assert(false);
    }
  ...
  __host__ __device__
      EdgePos_t
      get_end_edge_idx(VertexID vertex_id) const
  {
    if (!(vertex_id < n_vertices && 0 <= vertex_id))
    {
      printf("vertex_id %ld\n", vertex_id);
  ...
  __host__ __device__ bool has_edge(VertexID u, VertexID v) const
  {
  ...
  __host__ __device__ const CSR::Edge *get_edges() const { return &edges[0]; }
  __host__ __device__ const float *get_weights() const
  {
    return &weights[0];
  }

  __host__ __device__ const CSR::Vertex *get_vertices() const { return &vertices[0]; }

  __host__ __device__
      VertexID
      get_n_vertices() const { return n_vertices; }

  __host__ __device__ void copy_vertices(CSR *src, int start, int end)
  {
  ...
  __host__ __device__ void copy_edges(CSR *src, EdgePos_t start, EdgePos_t end)
  {
  ...
  __host__ __device__ int get_n_edges() const { return n_edges; }
  ...
  __host__ bool has_vertex(VertexID v) const
  {
...
public:
  ...
  CSRPartition(int _start, int _end, EdgePos_t _edge_start_idx, EdgePos_t _edge_end_idx, const CSR::Vertex *_vertices, const CSR::Edge *_edges,
               const float *_weights) : first_vertex_id(_start), last_vertex_id(_end), vertices(_vertices), edges(_edges),
  ...
  __host__ __device__ __forceinline__
      EdgePos_t
      get_end_edge_idx(int vertex_id) const
  {
    assert(vertex_id <= last_vertex_id && first_vertex_id <= vertex_id);
    return vertices[vertex_id - first_vertex_id].end_edge_id;
  }

  __host__ __device__ __forceinline__ float get_max_weight(int vertex_id) const
  {
    assert(vertex_id <= last_vertex_id && first_vertex_id <= vertex_id);
  ...
  __host__ __device__ __forceinline__
      CSR::Edge
      get_edge(EdgePos_t idx) const
  {
    assert(idx >= first_edge_idx && idx <= last_edge_idx);
    return edges[idx - first_edge_idx];
  }

  __host__ __device__ __forceinline__ float get_weight(EdgePos_t idx) const
  {
    assert(idx >= first_edge_idx && idx <= last_edge_idx);
  ...
  __host__ __device__ __forceinline__ const CSR::Edge *get_edges() const
  {
    return edges;
  }

  __host__ __device__ __forceinline__ const float *get_weights() const
  {
    return weights;
  }

  __host__ __device__ __forceinline__ const CSR::Edge *get_edges(VertexID v) const
  {
    return &edges[get_start_edge_idx(v) - first_edge_idx];
  }

  __host__ __device__ __forceinline__ const float *get_weights(VertexID v) const
  {
    return &weights[get_start_edge_idx(v) - first_edge_idx];
  }

  __host__ __device__ __forceinline__ const CSR::Vertex *get_vertices() const
  {
    return vertices;
  }
  ...
  __host__ __device__ __forceinline__ bool has_vertex(int v) const
  {
    return v >= first_vertex_id && v <= last_vertex_id;
  }

  __host__ __device__ __forceinline__ bool has_edge(VertexID src, VertexID dst) const
  {
  ...
  __host__ __device__ __forceinline__ bool has_edge_logn(VertexID src, VertexID dst) const
  {
  ...
  __host__ __device__ __forceinline__
      VertexID
      get_vertex_idx(VertexID v) const
  {
    assert(has_vertex(v));
  ...
  __device__ __host__ const CSR::Vertex *get_vertex(VertexID id) const { return &vertices[id]; }
