...
class sampleGCN(GCN):
    ...
    def __init__(self, input_dim, output_dim, hidden_dim, step_num,
        ...
        self.edge = nn.Parameter(torch.ones(
            self.node_num+1, sample_scope, dtype=torch.float))
    ...
    def sample_loss(self, loss_up):
        ...
        logp = self.get_policy(self.batch_sampler).log_prob(
            self.batch_sampled.transpose(0, 1)).transpose(0, 1)
        batch_sampled_node = torch.gather(
            self.edge[self.batch_sampler], dim=1, index=self.batch_sampled).type(torch.int64)
    ...
    def weight_function(self, source_idx):
        ...
        source_ = self.feature[source_idx].unsqueeze(
            1).expand(-1, self.sample_scope, -1).to(self.device)
    ...
    def get_action(self, target_nodes):
        ...
        temp = policy.sample_n(self.sample_num)
    ...
    def sampleNodes(self, nodes):
        ...
        for i in range(self.step_num):
            ...
            out_nodes, adj = computeSubGraph(
                self.edge, self.weight, in_nodes, out_nodes)
            if layer == 0:
