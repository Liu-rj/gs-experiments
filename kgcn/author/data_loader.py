...
def dataset_split(rating_np, args):
    ...
    eval_indices = np.random.choice(list(range(n_ratings)), size=int(
    ...
    if args.ratio < 1:
        train_indices = np.random.choice(list(train_indices), size=int(
            len(train_indices) * args.ratio), replace=False)
...
def construct_adj(args, kg, entity_num):
    ...
    adj_entity = np.zeros(
        [entity_num, args.neighbor_sample_size], dtype=np.int64)
    ...
    for entity in range(entity_num):
        ...
        if n_neighbors >= args.neighbor_sample_size:
            sampled_indices = np.random.choice(
                list(range(n_neighbors)), size=args.neighbor_sample_size, replace=False)
        else:
            sampled_indices = np.random.choice(
                list(range(n_neighbors)), size=args.neighbor_sample_size, replace=True)
